// Microbe → drug map. Keyed by section + organism slug. The drug strings are
// drug slugs from drug-data.ts (so they cross-link to /pharmacology/[slug]).
// `strengths` are free-text virulence factors / defenses shown on the detail
// page. Organisms not listed render "Not characterized in this pokédex".

import type { SectionKey } from "./sections";

export type MicrobePharmacology = {
  section: SectionKey;
  slug: string;
  resistance: string[];
  effective: string[];
  /** Virulence factors / defenses that make the organism formidable. */
  strengths?: string[];
};

export const MICROBE_PHARMACOLOGY: MicrobePharmacology[] = [
  // ---------- Bacteria ----------
  {
    section: "bacteria",
    slug: "staphylococcus-aureus",
    resistance: ["penicillin-g", "ampicillin", "amoxicillin"],
    effective: [
      "nafcillin",
      "dicloxacillin",
      "cefazolin",
      "vancomycin",
      "daptomycin",
      "linezolid",
      "tedizolid",
      "dalbavancin",
      "clindamycin",
      "tmp-smx",
      "doxycycline",
      "minocycline",
      "mupirocin",
      "ceftaroline",
    ],
    strengths: [
      "Protein A (binds antibody Fc)",
      "Coagulase",
      "Penicillinase",
      "Biofilm formation",
      "TSST-1 / enterotoxin superantigens",
      "Hemolysins & leukocidins",
    ],
  },
  {
    section: "bacteria",
    slug: "staphylococcus-epidermidis",
    resistance: ["penicillin-g", "nafcillin", "cefazolin"],
    effective: [
      "vancomycin",
      "linezolid",
      "tedizolid",
      "dalbavancin",
      "daptomycin",
    ],
    strengths: [
      "Biofilm (slime) on catheters & prostheses",
      "Adheres to plastic/polymers",
      "Skin commensal reservoir",
    ],
  },
  {
    section: "bacteria",
    slug: "streptococcus-pyogenes",
    resistance: [],
    effective: [
      "penicillin-g",
      "penicillin-v",
      "amoxicillin",
      "azithromycin",
      "erythromycin",
      "cefazolin",
      "clindamycin",
    ],
    strengths: [
      "M protein (antiphagocytic)",
      "Hyaluronic acid capsule",
      "Streptolysins O & S",
      "Pyrogenic exotoxin (superantigen)",
    ],
  },
  {
    section: "bacteria",
    slug: "streptococcus-pneumoniae",
    resistance: ["penicillin-g"],
    effective: [
      "ceftriaxone",
      "amoxicillin",
      "azithromycin",
      "erythromycin",
      "levofloxacin",
      "moxifloxacin",
      "vancomycin",
    ],
    strengths: [
      "Polysaccharide capsule (antiphagocytic)",
      "IgA protease",
      "Pneumolysin",
    ],
  },
  {
    section: "bacteria",
    slug: "streptococcus-agalactiae",
    resistance: [],
    effective: ["penicillin-g", "penicillin-v", "ampicillin", "cefazolin"],
    strengths: ["Polysaccharide capsule", "Beta-hemolysin/cytolysin"],
  },
  {
    section: "bacteria",
    slug: "enterococcus-faecalis",
    resistance: ["cefazolin", "ceftriaxone", "aztreonam", "tmp-smx"],
    effective: ["ampicillin", "vancomycin", "linezolid"],
    strengths: [
      "Bile & 6.5% NaCl tolerant",
      "Biofilm on heart valves",
      "Intrinsic cephalosporin resistance",
    ],
  },
  {
    section: "bacteria",
    slug: "enterococcus-faecium",
    resistance: ["ampicillin", "vancomycin", "penicillin-g"],
    effective: ["linezolid", "tedizolid", "daptomycin"],
    strengths: [
      "VanA glycopeptide resistance (VRE)",
      "Ampicillin resistance",
      "Hospital persistence",
    ],
  },
  {
    section: "bacteria",
    slug: "clostridioides-difficile",
    resistance: ["ceftriaxone", "clindamycin"],
    effective: ["fidaxomicin", "vancomycin", "metronidazole"],
    strengths: [
      "Alcohol-resistant spores",
      "Toxin A (enterotoxin)",
      "Toxin B (cytotoxin)",
    ],
  },
  {
    section: "bacteria",
    slug: "clostridium-tetani",
    resistance: [],
    effective: ["metronidazole", "penicillin-g"],
    strengths: ["Terminal spores", "Tetanospasmin (blocks GABA/glycine)"],
  },
  {
    section: "bacteria",
    slug: "clostridium-perfringens",
    resistance: [],
    effective: ["penicillin-g", "clindamycin"],
    strengths: [
      "Alpha toxin (lecithinase)",
      "Gas production in tissue",
      "Spores",
    ],
  },
  {
    section: "bacteria",
    slug: "bacillus-anthracis",
    resistance: [],
    effective: ["ciprofloxacin", "doxycycline", "penicillin-g"],
    strengths: [
      "Poly-D-glutamate capsule",
      "Edema & lethal toxins",
      "Durable spores",
    ],
  },
  {
    section: "bacteria",
    slug: "listeria-monocytogenes",
    resistance: ["ceftriaxone", "cefepime", "ceftazidime"],
    effective: ["ampicillin", "tmp-smx", "gentamicin"],
    strengths: [
      "Actin-based intracellular motility",
      "Listeriolysin O (escapes phagosome)",
      "Grows at 4°C",
    ],
  },
  {
    section: "bacteria",
    slug: "corynebacterium-diphtheriae",
    resistance: [],
    effective: ["penicillin-g", "azithromycin", "erythromycin"],
    strengths: ["Diphtheria toxin (ADP-ribosylates EF-2)", "Pseudomembrane"],
  },
  {
    section: "bacteria",
    slug: "mycobacterium-tuberculosis",
    resistance: ["penicillin-g", "ceftriaxone", "tmp-smx"],
    effective: [
      "isoniazid",
      "rifampin",
      "pyrazinamide",
      "ethambutol",
      "moxifloxacin",
      "levofloxacin",
      "amikacin",
      "bedaquiline",
      "linezolid",
    ],
    strengths: [
      "Mycolic-acid (acid-fast) wall",
      "Cord factor",
      "Inhibits phagolysosome fusion",
      "Intracellular survival & latency",
    ],
  },
  {
    section: "bacteria",
    slug: "mycobacterium-leprae",
    resistance: [],
    effective: ["rifampin", "dapsone", "clofazimine"],
    strengths: [
      "Grows in cool tissues (skin/nerves)",
      "Intracellular in macrophages & Schwann cells",
      "Uncultivable in vitro",
    ],
  },
  {
    section: "bacteria",
    slug: "escherichia-coli",
    resistance: ["ampicillin", "penicillin-g"],
    effective: [
      "ceftriaxone",
      "ceftazidime-avibactam",
      "meropenem",
      "ertapenem",
      "imipenem-cilastatin",
      "piperacillin-tazobactam",
      "aztreonam",
      "ciprofloxacin",
      "tmp-smx",
      "nitrofurantoin",
      "fosfomycin",
    ],
    strengths: [
      "Endotoxin (lipid A)",
      "Adhesive fimbriae (P / type 1 pili)",
      "K1 capsule (neonatal meningitis)",
      "ESBL in resistant strains",
    ],
  },
  {
    section: "bacteria",
    slug: "salmonella-enterica",
    resistance: [],
    effective: ["ceftriaxone", "ciprofloxacin", "azithromycin", "chloramphenicol"],
    strengths: [
      "Vi capsule (Typhi)",
      "Acid-tolerance response",
      "Intracellular in macrophages",
      "Type III secretion",
    ],
  },
  {
    section: "bacteria",
    slug: "shigella-dysenteriae",
    resistance: ["ampicillin", "tmp-smx"],
    effective: ["ciprofloxacin", "azithromycin", "ceftriaxone"],
    strengths: [
      "Shiga toxin",
      "Cell-to-cell spread (actin)",
      "Very low infectious dose",
      "Acid resistance",
    ],
  },
  {
    section: "bacteria",
    slug: "klebsiella-pneumoniae",
    resistance: ["ampicillin"],
    effective: [
      "ceftriaxone",
      "ceftazidime-avibactam",
      "meropenem",
      "ertapenem",
      "imipenem-cilastatin",
      "piperacillin-tazobactam",
      "aztreonam",
      "tigecycline",
    ],
    strengths: [
      "Thick polysaccharide capsule",
      "ESBL & carbapenemase (KPC)",
      "Hypervirulent mucoid strains",
    ],
  },
  {
    section: "bacteria",
    slug: "pseudomonas-aeruginosa",
    resistance: ["ampicillin", "ceftriaxone", "ertapenem", "tmp-smx"],
    effective: [
      "piperacillin-tazobactam",
      "ceftazidime",
      "ceftazidime-avibactam",
      "cefepime",
      "aztreonam",
      "meropenem",
      "imipenem-cilastatin",
      "ciprofloxacin",
      "gentamicin",
      "tobramycin",
      "amikacin",
      "colistin",
    ],
    strengths: [
      "Exotoxin A (inhibits EF-2)",
      "Pyocyanin (reactive oxygen)",
      "Alginate biofilm",
      "Efflux pumps & porin loss",
    ],
  },
  {
    section: "bacteria",
    slug: "acinetobacter-baumannii",
    resistance: ["ampicillin", "ceftriaxone", "ertapenem"],
    effective: [
      "meropenem",
      "imipenem-cilastatin",
      "minocycline",
      "tigecycline",
      "amikacin",
      "colistin",
    ],
    strengths: [
      "Survives desiccation on surfaces",
      "OXA carbapenemases",
      "Biofilm; pan-drug resistance",
    ],
  },
  {
    section: "bacteria",
    slug: "stenotrophomonas-maltophilia",
    resistance: [
      "meropenem",
      "imipenem-cilastatin",
      "piperacillin-tazobactam",
      "ceftazidime",
    ],
    effective: ["tmp-smx", "levofloxacin", "moxifloxacin", "minocycline"],
    strengths: [
      "Intrinsic carbapenem resistance",
      "L1 metallo-β-lactamase",
      "Biofilm on devices",
    ],
  },
  {
    section: "bacteria",
    slug: "haemophilus-influenzae",
    resistance: ["ampicillin"],
    effective: [
      "ceftriaxone",
      "amoxicillin-clavulanate",
      "cefuroxime",
      "azithromycin",
      "moxifloxacin",
    ],
    strengths: [
      "PRP capsule (type b)",
      "IgA protease",
      "β-lactamase (many strains)",
    ],
  },
  {
    section: "bacteria",
    slug: "neisseria-gonorrhoeae",
    resistance: ["penicillin-g", "ciprofloxacin", "tmp-smx", "tetracycline"],
    effective: ["ceftriaxone", "azithromycin"],
    strengths: [
      "Pilus antigenic & phase variation",
      "IgA protease",
      "No protective immunity",
      "Survives within neutrophils",
    ],
  },
  {
    section: "bacteria",
    slug: "neisseria-meningitidis",
    resistance: [],
    effective: ["ceftriaxone", "penicillin-g", "rifampin", "ciprofloxacin"],
    strengths: [
      "Polysaccharide capsule",
      "Endotoxin (LOS) blebs",
      "IgA protease",
    ],
  },
  {
    section: "bacteria",
    slug: "helicobacter-pylori",
    resistance: [],
    effective: ["amoxicillin", "clarithromycin", "metronidazole", "tetracycline"],
    strengths: [
      "Urease (neutralizes acid)",
      "Flagellar motility in mucus",
      "CagA / VacA cytotoxins",
    ],
  },
  {
    section: "bacteria",
    slug: "campylobacter-jejuni",
    resistance: [],
    effective: ["azithromycin", "erythromycin", "ciprofloxacin"],
    strengths: [
      "Darting flagellar motility",
      "LOS mimics gangliosides (triggers GBS)",
      "Microaerophilic",
    ],
  },
  {
    section: "bacteria",
    slug: "vibrio-cholerae",
    resistance: [],
    effective: ["doxycycline", "azithromycin", "ciprofloxacin"],
    strengths: [
      "Cholera toxin (↑ cAMP)",
      "Toxin-coregulated pili",
      "Single polar flagellum",
    ],
  },
  {
    section: "bacteria",
    slug: "treponema-pallidum",
    resistance: [],
    effective: ["penicillin-g", "doxycycline", "ceftriaxone"],
    strengths: [
      "Antigenically inert outer membrane",
      "Endoflagellar motility",
      "Crosses the placenta",
    ],
  },
  {
    section: "bacteria",
    slug: "borrelia-burgdorferi",
    resistance: [],
    effective: ["doxycycline", "ceftriaxone", "amoxicillin"],
    strengths: [
      "VlsE antigenic variation",
      "Tissue dissemination",
      "Immune evasion",
    ],
  },
  {
    section: "bacteria",
    slug: "leptospira-interrogans",
    resistance: [],
    effective: ["doxycycline", "penicillin-g", "ceftriaxone"],
    strengths: ["Corkscrew motility", "Penetrates intact skin/mucosa"],
  },
  {
    section: "bacteria",
    slug: "legionella-pneumophila",
    resistance: ["penicillin-g", "ampicillin"],
    effective: ["azithromycin", "erythromycin", "levofloxacin", "moxifloxacin"],
    strengths: [
      "Replicates inside macrophages",
      "Inhibits phagolysosome fusion",
      "Biofilm in water systems",
    ],
  },
  {
    section: "bacteria",
    slug: "chlamydia-trachomatis",
    resistance: ["penicillin-g", "ampicillin", "ceftriaxone"],
    effective: ["azithromycin", "doxycycline", "erythromycin"],
    strengths: [
      "Obligate intracellular",
      "Inhibits phagolysosome fusion",
      "No classic peptidoglycan",
    ],
  },
  {
    section: "bacteria",
    slug: "mycoplasma-pneumoniae",
    resistance: [
      "penicillin-g",
      "ampicillin",
      "ceftriaxone",
      "cefazolin",
      "vancomycin",
    ],
    effective: [
      "azithromycin",
      "erythromycin",
      "doxycycline",
      "levofloxacin",
      "moxifloxacin",
    ],
    strengths: [
      "No cell wall (β-lactam resistant)",
      "P1 adhesin",
      "Cold-agglutinin induction",
    ],
  },
  {
    section: "bacteria",
    slug: "bordetella-pertussis",
    resistance: [],
    effective: ["azithromycin", "clarithromycin", "erythromycin"],
    strengths: [
      "Pertussis toxin",
      "Tracheal cytotoxin",
      "Filamentous hemagglutinin adhesin",
    ],
  },
  {
    section: "bacteria",
    slug: "bacteroides-fragilis",
    resistance: ["penicillin-g", "cefazolin", "aztreonam", "gentamicin"],
    effective: [
      "metronidazole",
      "piperacillin-tazobactam",
      "meropenem",
      "ertapenem",
      "imipenem-cilastatin",
      "cefoxitin",
      "clindamycin",
    ],
    strengths: [
      "Polysaccharide capsule (abscess)",
      "β-lactamase",
      "Obligate anaerobe",
    ],
  },
  {
    section: "bacteria",
    slug: "rickettsia-rickettsii",
    resistance: ["penicillin-g"],
    effective: ["doxycycline", "chloramphenicol"],
    strengths: [
      "Obligate intracellular",
      "Actin-based motility",
      "Endothelial tropism (vasculitis)",
    ],
  },
  {
    section: "bacteria",
    slug: "anaplasma-phagocytophilum",
    resistance: [],
    effective: ["doxycycline"],
    strengths: ["Intracellular in neutrophils (morulae)", "Inhibits apoptosis"],
  },
  {
    section: "bacteria",
    slug: "ehrlichia-chaffeensis",
    resistance: [],
    effective: ["doxycycline"],
    strengths: ["Intracellular in monocytes (morulae)"],
  },
  {
    section: "bacteria",
    slug: "burkholderia-pseudomallei",
    resistance: ["penicillin-g", "ampicillin", "gentamicin", "colistin"],
    effective: [
      "ceftazidime",
      "meropenem",
      "imipenem-cilastatin",
      "tmp-smx",
      "amoxicillin-clavulanate",
    ],
    strengths: [
      "Facultative intracellular",
      "Latency & relapse",
      "Intrinsic aminoglycoside/colistin resistance",
    ],
  },
  {
    section: "bacteria",
    slug: "nocardia-asteroides",
    resistance: [],
    effective: ["tmp-smx", "linezolid", "meropenem", "imipenem-cilastatin", "minocycline", "amikacin"],
    strengths: [
      "Catalase & superoxide dismutase",
      "Partially acid-fast",
      "Intracellular survival",
    ],
  },
  {
    section: "bacteria",
    slug: "actinomyces-israelii",
    resistance: [],
    effective: ["penicillin-g", "amoxicillin", "doxycycline", "clindamycin"],
    strengths: [
      "Sulfur-granule colonies",
      "Forms draining sinus tracts",
      "Anaerobic biofilm",
    ],
  },
  {
    section: "bacteria",
    slug: "borrelia-recurrentis",
    resistance: [],
    effective: ["doxycycline", "erythromycin", "penicillin-g"],
    strengths: ["Antigenic variation (relapsing fever)", "Louse-borne"],
  },
  {
    section: "bacteria",
    slug: "clostridium-botulinum",
    resistance: [],
    effective: ["penicillin-g", "metronidazole"],
    strengths: [
      "Botulinum neurotoxin (blocks ACh release)",
      "Heat-resistant spores",
      "Flaccid paralysis",
    ],
  },
  {
    section: "bacteria",
    slug: "bacillus-cereus",
    resistance: ["penicillin-g", "ampicillin", "cefazolin"],
    effective: ["vancomycin", "clindamycin", "ciprofloxacin"],
    strengths: [
      "Heat-stable emetic toxin (cereulide)",
      "Diarrheal enterotoxin",
      "Spores survive reheating",
    ],
  },
  {
    section: "bacteria",
    slug: "yersinia-pestis",
    resistance: [],
    effective: ["streptomycin", "gentamicin", "doxycycline", "ciprofloxacin"],
    strengths: [
      "F1 capsular antigen (antiphagocytic)",
      "Type III secretion (Yops)",
      "Facultative intracellular survival",
    ],
  },
  {
    section: "bacteria",
    slug: "proteus-mirabilis",
    resistance: ["nitrofurantoin", "tetracycline", "colistin"],
    effective: ["ceftriaxone", "tmp-smx", "ciprofloxacin", "ampicillin"],
    strengths: [
      "Swarming motility",
      "Urease (struvite stones)",
      "Alkalinizes urine",
    ],
  },
  {
    section: "bacteria",
    slug: "serratia-marcescens",
    resistance: ["ampicillin", "amoxicillin", "cefazolin", "colistin"],
    effective: ["cefepime", "meropenem", "ciprofloxacin", "gentamicin"],
    strengths: [
      "Inducible AmpC β-lactamase",
      "Red prodigiosin pigment",
      "Biofilm on devices",
    ],
  },
  {
    section: "bacteria",
    slug: "moraxella-catarrhalis",
    resistance: ["penicillin-g", "ampicillin"],
    effective: [
      "amoxicillin-clavulanate",
      "ceftriaxone",
      "azithromycin",
      "tmp-smx",
    ],
    strengths: [
      "BRO β-lactamase",
      "Adhesins (otitis/sinusitis/COPD)",
      "Complement resistance",
    ],
  },
  {
    section: "bacteria",
    slug: "staphylococcus-saprophyticus",
    resistance: [],
    effective: ["tmp-smx", "nitrofurantoin", "ciprofloxacin"],
    strengths: [
      "Novobiocin resistance",
      "Adheres to uroepithelium",
      "Urease",
    ],
  },
  {
    section: "bacteria",
    slug: "cutibacterium-acnes",
    resistance: ["metronidazole"],
    effective: ["doxycycline", "clindamycin", "penicillin-g"],
    strengths: [
      "Lipase (drives acne)",
      "Biofilm on implants/shoulder hardware",
      "Slow-growing anaerobe",
    ],
  },
  {
    section: "bacteria",
    slug: "gardnerella-vaginalis",
    resistance: [],
    effective: ["metronidazole", "clindamycin"],
    strengths: [
      "Biofilm coating clue cells",
      "Raises vaginal pH",
      "Polymicrobial synergy",
    ],
  },
  {
    section: "bacteria",
    slug: "mycobacterium-avium",
    resistance: ["isoniazid", "pyrazinamide"],
    effective: [
      "clarithromycin",
      "azithromycin",
      "ethambutol",
      "rifampin",
      "amikacin",
    ],
    strengths: [
      "Biofilm in water systems",
      "Intracellular in macrophages",
      "Disseminates in AIDS (CD4 < 50)",
    ],
  },
  {
    section: "bacteria",
    slug: "coxiella-burnetii",
    resistance: ["penicillin-g"],
    effective: ["doxycycline", "hydroxychloroquine"],
    strengths: [
      "Spore-like, environmentally stable",
      "Obligate intracellular (acidic vacuole)",
      "Very low infectious dose",
      "Phase variation",
    ],
  },
  {
    section: "bacteria",
    slug: "bartonella-henselae",
    resistance: [],
    effective: ["azithromycin", "doxycycline", "rifampin"],
    strengths: [
      "Invades endothelium & red cells",
      "Bacillary angiomatosis (vasoproliferation)",
      "Cat-scratch transmission",
    ],
  },
  {
    section: "bacteria",
    slug: "francisella-tularensis",
    resistance: ["penicillin-g"],
    effective: ["streptomycin", "gentamicin", "doxycycline", "ciprofloxacin"],
    strengths: [
      "Extremely low infectious dose",
      "Facultative intracellular",
      "Anti-phagocytic capsule",
    ],
  },
  {
    section: "bacteria",
    slug: "brucella-melitensis",
    resistance: [],
    effective: ["doxycycline", "rifampin", "streptomycin", "gentamicin"],
    strengths: [
      "Survives inside macrophages",
      "Inhibits phagosome killing",
      "Undulant relapse",
    ],
  },
  {
    section: "bacteria",
    slug: "vibrio-vulnificus",
    resistance: [],
    effective: ["doxycycline", "ceftriaxone", "ciprofloxacin"],
    strengths: [
      "Polysaccharide capsule",
      "Iron-dependent fulminance (liver disease)",
      "Hemolysins → hemorrhagic bullae",
    ],
  },
  {
    section: "bacteria",
    slug: "yersinia-enterocolitica",
    resistance: ["penicillin-g", "ampicillin"],
    effective: ["tmp-smx", "ciprofloxacin", "ceftriaxone", "doxycycline"],
    strengths: [
      "Grows at refrigeration temps",
      "Type III secretion",
      "Mesenteric adenitis (pseudoappendicitis)",
    ],
  },
  {
    section: "bacteria",
    slug: "pasteurella-multocida",
    resistance: [],
    effective: ["amoxicillin-clavulanate", "penicillin-g", "doxycycline"],
    strengths: [
      "Polysaccharide capsule",
      "Rapid cellulitis after animal bites",
      "Endotoxin",
    ],
  },
  {
    section: "bacteria",
    slug: "fusobacterium-nucleatum",
    resistance: [],
    effective: ["metronidazole", "penicillin-g", "clindamycin"],
    strengths: [
      "Anaerobic adhesins (co-aggregation)",
      "Periodontal disease & abscesses",
      "Linked to colorectal tumors",
    ],
  },
  {
    section: "bacteria",
    slug: "burkholderia-cepacia",
    resistance: ["gentamicin", "colistin", "polymyxin-b"],
    effective: ["tmp-smx", "meropenem", "ceftazidime", "minocycline"],
    strengths: [
      "Multidrug efflux pumps",
      "Biofilm in cystic-fibrosis lungs",
      "Survives in disinfectants",
    ],
  },
  {
    section: "bacteria",
    slug: "streptococcus-gallolyticus",
    resistance: [],
    effective: ["penicillin-g", "ceftriaxone", "vancomycin"],
    strengths: [
      "Bile tolerance",
      "Colorectal adenocarcinoma association",
      "Valve adherence (endocarditis)",
    ],
  },
  {
    section: "bacteria",
    slug: "erysipelothrix-rhusiopathiae",
    resistance: ["vancomycin"],
    effective: ["penicillin-g", "ceftriaxone", "ciprofloxacin"],
    strengths: [
      "Intrinsic vancomycin resistance",
      "Neuraminidase & hyaluronidase",
      "Erysipeloid in animal handlers",
    ],
  },
  {
    section: "bacteria",
    slug: "rhodococcus-equi",
    resistance: ["penicillin-g"],
    effective: [
      "vancomycin",
      "azithromycin",
      "rifampin",
      "ciprofloxacin",
      "imipenem-cilastatin",
    ],
    strengths: [
      "Intracellular in macrophages",
      "Partially acid-fast (mycolic wall)",
      "Cavitary pneumonia in HIV",
    ],
  },
  {
    section: "bacteria",
    slug: "tropheryma-whipplei",
    resistance: [],
    effective: ["ceftriaxone", "tmp-smx", "doxycycline"],
    strengths: [
      "Intracellular in macrophages (PAS-positive)",
      "Immune evasion",
      "Indolent multisystem disease",
    ],
  },
  {
    section: "bacteria",
    slug: "mycobacterium-abscessus",
    resistance: ["isoniazid", "rifampin", "ethambutol"],
    effective: [
      "clarithromycin",
      "azithromycin",
      "amikacin",
      "cefoxitin",
      "tigecycline",
      "linezolid",
    ],
    strengths: [
      "Rapid-growing & highly drug-resistant",
      "Biofilm formation",
      "Inducible macrolide resistance (erm)",
    ],
  },
  {
    section: "bacteria",
    slug: "vibrio-parahaemolyticus",
    resistance: [],
    effective: ["doxycycline", "ciprofloxacin", "ceftriaxone"],
    strengths: [
      "Kanagawa hemolysin",
      "Halophilic marine survival",
      "Type III secretion",
    ],
  },
  {
    section: "bacteria",
    slug: "aeromonas-hydrophila",
    resistance: ["ampicillin"],
    effective: ["ciprofloxacin", "tmp-smx", "ceftriaxone"],
    strengths: [
      "Cytotoxins & hemolysins",
      "Freshwater wound infection",
      "Multiple β-lactamases",
    ],
  },
  {
    section: "bacteria",
    slug: "citrobacter-freundii",
    resistance: ["ampicillin", "amoxicillin", "cefazolin"],
    effective: ["cefepime", "meropenem", "ciprofloxacin", "tmp-smx"],
    strengths: [
      "Inducible AmpC β-lactamase",
      "Neonatal meningitis / brain abscess",
      "Biofilm",
    ],
  },
  {
    section: "bacteria",
    slug: "enterobacter-cloacae",
    resistance: ["ampicillin", "amoxicillin", "cefazolin", "cefoxitin"],
    effective: ["cefepime", "meropenem", "ciprofloxacin", "tmp-smx"],
    strengths: [
      "Inducible AmpC (ceftriaxone failure)",
      "Hospital persistence",
      "Biofilm on lines",
    ],
  },
  {
    section: "bacteria",
    slug: "morganella-morganii",
    resistance: ["ampicillin", "nitrofurantoin", "colistin", "tetracycline"],
    effective: ["cefepime", "meropenem", "ciprofloxacin", "tmp-smx"],
    strengths: [
      "Inducible AmpC β-lactamase",
      "Urease",
      "Catheter-associated UTI",
    ],
  },
  {
    section: "bacteria",
    slug: "eikenella-corrodens",
    resistance: ["clindamycin", "metronidazole"],
    effective: ["amoxicillin-clavulanate", "penicillin-g", "ceftriaxone"],
    strengths: [
      "HACEK endocarditis",
      "Pits agar; 'fight-bite' infections",
      "Oral flora",
    ],
  },
  {
    section: "bacteria",
    slug: "kingella-kingae",
    resistance: [],
    effective: ["ceftriaxone", "penicillin-g", "amoxicillin"],
    strengths: [
      "HACEK organism",
      "Pediatric septic arthritis/osteomyelitis",
      "Adhesive type IV pili",
    ],
  },
  {
    section: "bacteria",
    slug: "capnocytophaga-canimorsus",
    resistance: [],
    effective: ["amoxicillin-clavulanate", "penicillin-g", "meropenem"],
    strengths: [
      "Fulminant sepsis in asplenia",
      "Dog/cat-bite exposure",
      "Evades phagocytosis",
    ],
  },
  {
    section: "bacteria",
    slug: "porphyromonas-gingivalis",
    resistance: [],
    effective: ["metronidazole", "amoxicillin-clavulanate", "clindamycin"],
    strengths: [
      "Gingipain proteases",
      "Keystone periodontal pathogen",
      "Subverts complement",
    ],
  },
  {
    section: "bacteria",
    slug: "ureaplasma-urealyticum",
    resistance: ["penicillin-g", "ampicillin", "ceftriaxone", "vancomycin"],
    effective: ["azithromycin", "doxycycline", "levofloxacin"],
    strengths: [
      "No cell wall (β-lactam resistant)",
      "Urease",
      "Nongonococcal urethritis",
    ],
  },
  {
    section: "bacteria",
    slug: "haemophilus-ducreyi",
    resistance: ["penicillin-g"],
    effective: ["azithromycin", "ceftriaxone", "ciprofloxacin", "erythromycin"],
    strengths: [
      "Cytolethal distending toxin",
      "Painful genital ulcers (chancroid)",
      "'School-of-fish' clusters",
    ],
  },
  {
    section: "bacteria",
    slug: "aggregatibacter-actinomycetemcomitans",
    resistance: [],
    effective: ["amoxicillin-clavulanate", "ceftriaxone", "doxycycline"],
    strengths: [
      "Leukotoxin",
      "HACEK endocarditis",
      "Aggressive periodontitis",
    ],
  },
  {
    section: "bacteria",
    slug: "cardiobacterium-hominis",
    resistance: [],
    effective: ["ceftriaxone", "penicillin-g", "amoxicillin"],
    strengths: [
      "HACEK endocarditis",
      "Pleomorphic, slow-growing",
      "Culture-elusive (extended incubation)",
    ],
  },
  {
    section: "bacteria",
    slug: "cronobacter-sakazakii",
    resistance: ["ampicillin"],
    effective: ["meropenem", "cefepime", "ciprofloxacin", "tmp-smx"],
    strengths: [
      "Survives in powdered infant formula",
      "Desiccation tolerance",
      "Neonatal meningitis / NEC",
    ],
  },
  {
    section: "bacteria",
    slug: "chromobacterium-violaceum",
    resistance: ["penicillin-g", "ampicillin", "cefazolin"],
    effective: ["ciprofloxacin", "meropenem", "tmp-smx"],
    strengths: [
      "Violacein pigment",
      "Fulminant sepsis in CGD",
      "Soil/water exposure",
    ],
  },
  {
    section: "bacteria",
    slug: "rickettsia-prowazekii",
    resistance: ["penicillin-g"],
    effective: ["doxycycline", "chloramphenicol"],
    strengths: [
      "Obligate intracellular",
      "Louse-borne epidemic typhus",
      "Brill-Zinsser recrudescence",
    ],
  },
  {
    section: "bacteria",
    slug: "orientia-tsutsugamushi",
    resistance: ["penicillin-g"],
    effective: ["doxycycline", "azithromycin", "chloramphenicol"],
    strengths: [
      "Obligate intracellular",
      "Chigger-borne; inoculation eschar",
      "Marked antigenic diversity",
    ],
  },
  {
    section: "bacteria",
    slug: "shigella-sonnei",
    resistance: ["ampicillin", "tmp-smx"],
    effective: ["ciprofloxacin", "azithromycin", "ceftriaxone"],
    strengths: [
      "Very low infectious dose",
      "Cell-to-cell actin spread",
      "Acid resistance",
    ],
  },
  {
    section: "bacteria",
    slug: "klebsiella-oxytoca",
    resistance: ["ampicillin"],
    effective: [
      "ceftriaxone",
      "meropenem",
      "piperacillin-tazobactam",
      "ciprofloxacin",
    ],
    strengths: [
      "Tilivalline cytotoxin (hemorrhagic colitis)",
      "Polysaccharide capsule",
      "ESBL strains",
    ],
  },
  {
    section: "bacteria",
    slug: "fusobacterium-necrophorum",
    resistance: [],
    effective: ["metronidazole", "penicillin-g", "piperacillin-tazobactam"],
    strengths: [
      "Lemierre's syndrome (jugular thrombophlebitis)",
      "Leukotoxin",
      "Anaerobic",
    ],
  },
  {
    section: "bacteria",
    slug: "staphylococcus-lugdunensis",
    resistance: ["penicillin-g"],
    effective: ["nafcillin", "cefazolin", "vancomycin"],
    strengths: [
      "Coagulase-negative but aggressive",
      "Destructive native-valve endocarditis",
      "Often methicillin-susceptible",
    ],
  },
  {
    section: "bacteria",
    slug: "streptococcus-anginosus",
    resistance: [],
    effective: ["penicillin-g", "ceftriaxone", "clindamycin", "vancomycin"],
    strengths: [
      "Abscess formation (brain/liver/thoracic)",
      "Caramel odor",
      "Microaerophilic",
    ],
  },
  {
    section: "bacteria",
    slug: "arcanobacterium-haemolyticum",
    resistance: ["tmp-smx"],
    effective: ["azithromycin", "erythromycin", "penicillin-g", "clindamycin"],
    strengths: [
      "Phospholipase D",
      "Pharyngitis with scarlatiniform rash",
      "Mimics strep throat",
    ],
  },
  {
    section: "bacteria",
    slug: "corynebacterium-jeikeium",
    resistance: ["penicillin-g", "cefazolin", "ceftriaxone", "gentamicin"],
    effective: ["vancomycin", "linezolid", "daptomycin"],
    strengths: [
      "Multidrug resistant",
      "Lipophilic skin colonizer",
      "Line sepsis in neutropenia",
    ],
  },
  {
    section: "bacteria",
    slug: "clostridium-septicum",
    resistance: [],
    effective: ["penicillin-g", "clindamycin", "metronidazole"],
    strengths: [
      "Alpha toxin",
      "Atraumatic gas gangrene (colon-cancer link)",
      "Aerotolerant spores",
    ],
  },
  {
    section: "bacteria",
    slug: "mycobacterium-marinum",
    resistance: ["isoniazid", "pyrazinamide"],
    effective: [
      "clarithromycin",
      "ethambutol",
      "rifampin",
      "doxycycline",
      "tmp-smx",
    ],
    strengths: [
      "Grows at cooler skin temperatures",
      "Fish-tank granuloma",
      "Photochromogen",
    ],
  },
  {
    section: "bacteria",
    slug: "mycobacterium-kansasii",
    resistance: ["pyrazinamide"],
    effective: [
      "rifampin",
      "isoniazid",
      "ethambutol",
      "clarithromycin",
      "azithromycin",
    ],
    strengths: [
      "Photochromogen",
      "TB-like cavitary lung disease",
      "Acid-fast wall",
    ],
  },
  {
    section: "bacteria",
    slug: "mycobacterium-bovis",
    resistance: ["pyrazinamide"],
    effective: ["isoniazid", "rifampin", "ethambutol"],
    strengths: [
      "Acid-fast (M. tuberculosis complex)",
      "GI TB from unpasteurized dairy",
      "Source of BCG vaccine",
    ],
  },
  {
    section: "bacteria",
    slug: "nocardia-brasiliensis",
    resistance: [],
    effective: ["tmp-smx", "amikacin", "minocycline", "linezolid"],
    strengths: [
      "Partially acid-fast branching",
      "Lymphocutaneous disease / mycetoma",
      "Catalase & superoxide dismutase",
    ],
  },
  {
    section: "bacteria",
    slug: "burkholderia-mallei",
    resistance: ["penicillin-g", "gentamicin", "colistin"],
    effective: [
      "ceftazidime",
      "meropenem",
      "imipenem-cilastatin",
      "tmp-smx",
      "doxycycline",
    ],
    strengths: [
      "Non-motile; intracellular",
      "Glanders (equine zoonosis)",
      "Aerosol-infectious",
    ],
  },
  {
    section: "bacteria",
    slug: "bartonella-quintana",
    resistance: [],
    effective: ["doxycycline", "gentamicin", "azithromycin"],
    strengths: [
      "Louse-borne trench fever",
      "Invades red cells & endothelium",
      "Bacillary angiomatosis",
    ],
  },
  {
    section: "bacteria",
    slug: "rickettsia-conorii",
    resistance: ["penicillin-g"],
    effective: ["doxycycline", "chloramphenicol"],
    strengths: [
      "Obligate intracellular",
      "Tick-borne; tache noire eschar",
      "Endothelial invasion",
    ],
  },
  {
    section: "bacteria",
    slug: "rickettsia-typhi",
    resistance: ["penicillin-g"],
    effective: ["doxycycline", "chloramphenicol"],
    strengths: [
      "Flea-borne endemic (murine) typhus",
      "Obligate intracellular",
      "Rat reservoir",
    ],
  },
  {
    section: "bacteria",
    slug: "mycoplasma-genitalium",
    resistance: ["penicillin-g", "ampicillin", "ceftriaxone", "vancomycin"],
    effective: ["azithromycin", "moxifloxacin", "doxycycline"],
    strengths: [
      "No cell wall (β-lactam resistant)",
      "Rising macrolide resistance",
      "Terminal adhesion organelle",
    ],
  },
  {
    section: "bacteria",
    slug: "mycoplasma-hominis",
    resistance: ["penicillin-g", "ampicillin", "ceftriaxone", "azithromycin"],
    effective: ["doxycycline", "moxifloxacin", "clindamycin"],
    strengths: [
      "No cell wall",
      "Intrinsic macrolide resistance",
      "Postpartum fever / PID",
    ],
  },
  {
    section: "bacteria",
    slug: "chlamydia-pneumoniae",
    resistance: ["penicillin-g", "ampicillin", "ceftriaxone"],
    effective: ["azithromycin", "doxycycline", "levofloxacin"],
    strengths: [
      "Obligate intracellular",
      "Atypical pneumonia",
      "No classic peptidoglycan",
    ],
  },
  {
    section: "bacteria",
    slug: "chlamydia-psittaci",
    resistance: ["penicillin-g", "ceftriaxone"],
    effective: ["doxycycline", "azithromycin"],
    strengths: [
      "Obligate intracellular",
      "Bird reservoir (psittacosis)",
      "Inhibits phagolysosome fusion",
    ],
  },
  {
    section: "bacteria",
    slug: "streptococcus-suis",
    resistance: [],
    effective: ["penicillin-g", "ceftriaxone", "ampicillin"],
    strengths: [
      "Polysaccharide capsule",
      "Suilysin",
      "Zoonotic meningitis/deafness (pork)",
    ],
  },
  {
    section: "bacteria",
    slug: "streptococcus-mutans",
    resistance: [],
    effective: ["penicillin-g", "vancomycin"],
    strengths: [
      "Dextran biofilm (dental plaque)",
      "Acidogenic (caries)",
      "Valve adherence (endocarditis)",
    ],
  },
  {
    section: "bacteria",
    slug: "streptococcus-sanguinis",
    resistance: [],
    effective: ["penicillin-g", "ceftriaxone", "vancomycin"],
    strengths: [
      "Plaque biofilm",
      "Dextran-mediated valve adherence",
      "Subacute endocarditis",
    ],
  },
  {
    section: "bacteria",
    slug: "streptococcus-mitis",
    resistance: [],
    effective: ["penicillin-g", "ceftriaxone", "vancomycin"],
    strengths: [
      "Viridans biofilm",
      "Neutropenic bacteremia",
      "Valve adherence",
    ],
  },
  {
    section: "bacteria",
    slug: "staphylococcus-haemolyticus",
    resistance: ["penicillin-g", "nafcillin"],
    effective: ["vancomycin", "linezolid", "daptomycin"],
    strengths: [
      "Coagulase-negative; glycopeptide creep",
      "Catheter & prosthetic infection",
      "Biofilm",
    ],
  },
  {
    section: "bacteria",
    slug: "granulicatella-adiacens",
    resistance: [],
    effective: ["penicillin-g", "ceftriaxone", "vancomycin", "gentamicin"],
    strengths: [
      "Nutritionally variant (pyridoxal-dependent)",
      "Culture-negative endocarditis",
    ],
  },
  {
    section: "bacteria",
    slug: "gemella-haemolysans",
    resistance: [],
    effective: ["penicillin-g", "ceftriaxone", "vancomycin"],
    strengths: [
      "Decolorizes easily (mimics Neisseria)",
      "Endocarditis",
      "Oral commensal",
    ],
  },
  {
    section: "bacteria",
    slug: "leuconostoc-mesenteroides",
    resistance: ["vancomycin"],
    effective: ["penicillin-g", "clindamycin"],
    strengths: [
      "Intrinsic vancomycin resistance",
      "Opportunist in immunocompromised",
    ],
  },
  {
    section: "bacteria",
    slug: "lactobacillus-rhamnosus",
    resistance: ["vancomycin"],
    effective: ["penicillin-g", "clindamycin"],
    strengths: [
      "Probiotic origin",
      "Intrinsic vancomycin resistance",
      "Rare endocarditis",
    ],
  },
  {
    section: "bacteria",
    slug: "mycobacterium-chelonae",
    resistance: ["isoniazid", "rifampin", "ethambutol"],
    effective: ["clarithromycin", "azithromycin", "tobramycin", "linezolid"],
    strengths: [
      "Rapid-growing acid-fast",
      "Post-procedure skin/soft-tissue infections",
      "Highly drug-resistant",
    ],
  },
  {
    section: "bacteria",
    slug: "mycobacterium-ulcerans",
    resistance: ["isoniazid", "pyrazinamide"],
    effective: ["rifampin", "clarithromycin", "streptomycin"],
    strengths: [
      "Mycolactone toxin (necrosis & immunosuppression)",
      "Buruli ulcer",
      "Painless ulceration",
    ],
  },
  {
    section: "bacteria",
    slug: "yersinia-pseudotuberculosis",
    resistance: ["penicillin-g", "ampicillin"],
    effective: ["tmp-smx", "ciprofloxacin", "ceftriaxone"],
    strengths: [
      "Mesenteric adenitis (pseudoappendicitis)",
      "Cold growth",
      "Superantigen (Far-East scarlatiniform fever)",
    ],
  },
  {
    section: "bacteria",
    slug: "plesiomonas-shigelloides",
    resistance: ["ampicillin"],
    effective: ["ciprofloxacin", "tmp-smx", "ceftriaxone"],
    strengths: [
      "Waterborne / seafood gastroenteritis",
      "Motile",
      "β-lactamase",
    ],
  },
  {
    section: "bacteria",
    slug: "edwardsiella-tarda",
    resistance: [],
    effective: ["ciprofloxacin", "ceftriaxone", "tmp-smx"],
    strengths: [
      "Fish & reptile exposure",
      "Wound & GI infection",
      "Hydrogen-sulfide production",
    ],
  },
  {
    section: "bacteria",
    slug: "providencia-stuartii",
    resistance: ["ampicillin", "nitrofurantoin", "colistin", "tetracycline"],
    effective: ["cefepime", "meropenem", "ciprofloxacin", "amikacin"],
    strengths: [
      "Catheter-associated UTI",
      "Urease",
      "Multidrug resistance",
    ],
  },
  {
    section: "bacteria",
    slug: "achromobacter-xylosoxidans",
    resistance: ["aztreonam", "cefepime"],
    effective: ["meropenem", "tmp-smx", "piperacillin-tazobactam"],
    strengths: [
      "Cystic-fibrosis airway colonizer",
      "Broad intrinsic resistance",
      "Biofilm",
    ],
  },
  {
    section: "bacteria",
    slug: "elizabethkingia-meningoseptica",
    resistance: ["meropenem", "ceftriaxone", "colistin", "aztreonam"],
    effective: ["levofloxacin", "minocycline", "tmp-smx"],
    strengths: [
      "Carbapenem-resistant (metallo-β-lactamase)",
      "Neonatal meningitis",
      "Waterborne nosocomial source",
    ],
  },
  {
    section: "bacteria",
    slug: "treponema-denticola",
    resistance: [],
    effective: ["metronidazole", "amoxicillin-clavulanate", "doxycycline"],
    strengths: [
      "Periodontal 'red complex'",
      "Spirochete motility",
      "Tissue proteases",
    ],
  },

  // ---------- Viruses ----------
  {
    section: "viruses",
    slug: "herpes-simplex-virus-1",
    resistance: [],
    effective: ["acyclovir", "valacyclovir", "famciclovir", "foscarnet"],
    strengths: [
      "Latency in sensory ganglia",
      "Reactivation under stress",
      "Evades MHC-I (ICP47)",
    ],
  },
  {
    section: "viruses",
    slug: "herpes-simplex-virus-2",
    resistance: [],
    effective: ["acyclovir", "valacyclovir", "famciclovir", "foscarnet"],
    strengths: [
      "Latency in sacral ganglia",
      "Asymptomatic shedding",
      "Evades MHC-I",
    ],
  },
  {
    section: "viruses",
    slug: "varicella-zoster-virus",
    resistance: [],
    effective: ["acyclovir", "valacyclovir", "famciclovir", "foscarnet"],
    strengths: [
      "Latency in dorsal root ganglia",
      "Airborne, highly contagious",
      "Reactivation as zoster",
    ],
  },
  {
    section: "viruses",
    slug: "cytomegalovirus",
    resistance: [],
    effective: ["ganciclovir", "letermovir", "foscarnet", "cidofovir"],
    strengths: [
      "Latency in myeloid cells",
      "Downregulates MHC-I",
      "Congenital transmission",
    ],
  },
  {
    section: "viruses",
    slug: "epstein-barr-virus",
    resistance: [],
    effective: [],
    strengths: [
      "Lifelong latency in B cells",
      "Immortalizes B cells (LMP1)",
      "Oncogenic (Burkitt, nasopharyngeal CA)",
    ],
  },
  {
    section: "viruses",
    slug: "influenza-a-virus",
    resistance: [],
    effective: ["oseltamivir", "baloxavir", "zanamivir", "peramivir"],
    strengths: [
      "Antigenic drift & shift",
      "Segment reassortment",
      "Pandemic potential",
    ],
  },
  {
    section: "viruses",
    slug: "influenza-b-virus",
    resistance: [],
    effective: ["oseltamivir", "baloxavir", "zanamivir", "peramivir"],
    strengths: ["Antigenic drift", "Human-restricted reservoir"],
  },
  {
    section: "viruses",
    slug: "sars-cov-2",
    resistance: [],
    effective: ["nirmatrelvir-ritonavir", "remdesivir", "molnupiravir"],
    strengths: [
      "Spike–ACE2 entry",
      "Rapid variant evolution",
      "Immune-escape mutations",
    ],
  },
  {
    section: "viruses",
    slug: "hiv-1",
    resistance: [],
    effective: [
      "dolutegravir",
      "bictegravir",
      "raltegravir",
      "tenofovir",
      "emtricitabine",
      "lamivudine",
      "zidovudine",
      "efavirenz",
      "nevirapine",
      "darunavir",
    ],
    strengths: [
      "Integrates as latent provirus",
      "Error-prone reverse transcriptase (escape)",
      "Depletes CD4 T cells",
      "gp120 antigenic variation",
    ],
  },
  {
    section: "viruses",
    slug: "hepatitis-b-virus",
    resistance: [],
    effective: ["tenofovir", "entecavir", "lamivudine"],
    strengths: [
      "Chronic carriage",
      "Oncogenic (HCC)",
      "Stable cccDNA reservoir",
    ],
  },
  {
    section: "viruses",
    slug: "hepatitis-c-virus",
    resistance: [],
    effective: [
      "sofosbuvir",
      "sofosbuvir-velpatasvir",
      "glecaprevir-pibrentasvir",
      "ribavirin",
    ],
    strengths: [
      "No proofreading (quasispecies escape)",
      "High chronicity",
      "Cirrhosis / HCC risk",
    ],
  },
  {
    section: "viruses",
    slug: "ebola-virus",
    resistance: [],
    effective: ["remdesivir", "ribavirin"],
    strengths: [
      "Suppresses interferon (VP35/VP24)",
      "Endothelial damage / hemorrhage",
      "Transmissible via body fluids",
    ],
  },
  {
    section: "viruses",
    slug: "monkeypox-virus",
    resistance: [],
    effective: ["tecovirimat", "cidofovir"],
    strengths: [
      "Cytokine-decoy immune evasion",
      "Environmentally stable",
      "Cytoplasmic replication",
    ],
  },
  {
    section: "viruses",
    slug: "variola-virus",
    resistance: [],
    effective: ["tecovirimat", "cidofovir"],
    strengths: [
      "Immune-evasion proteins",
      "Aerosol transmission",
      "Environmentally stable",
    ],
  },
  {
    section: "viruses",
    slug: "adenovirus",
    resistance: [],
    effective: ["cidofovir"],
    strengths: [
      "Non-enveloped (hardy/disinfectant-resistant)",
      "Broad tropism (eye, gut, lung)",
    ],
  },
  {
    section: "viruses",
    slug: "respiratory-syncytial-virus",
    resistance: [],
    effective: ["ribavirin"],
    strengths: [
      "Syncytium formation",
      "Reinfection throughout life",
      "Bronchiolar tropism",
    ],
  },
  {
    section: "viruses",
    slug: "lassa-virus",
    resistance: [],
    effective: ["ribavirin"],
    strengths: ["Suppresses innate immunity", "Persistent rodent reservoir"],
  },
  {
    section: "viruses",
    slug: "bk-polyomavirus",
    resistance: [],
    effective: ["cidofovir"],
    strengths: [
      "Latency in uroepithelium",
      "Reactivates in immunosuppression",
    ],
  },
  {
    section: "viruses",
    slug: "human-papillomavirus",
    resistance: [],
    effective: [],
    strengths: [
      "E6/E7 oncoproteins inactivate p53 & Rb",
      "Koilocytes",
      "Cervical & oropharyngeal cancer",
    ],
  },
  {
    section: "viruses",
    slug: "measles-virus",
    resistance: [],
    effective: [],
    strengths: [
      "Among the most contagious pathogens (R0 12–18)",
      "Immune amnesia",
      "Late SSPE",
    ],
  },
  {
    section: "viruses",
    slug: "rabies-virus",
    resistance: [],
    effective: [],
    strengths: [
      "Retrograde axonal transport to CNS",
      "Negri bodies",
      "Near-100% lethal once symptomatic",
    ],
  },
  {
    section: "viruses",
    slug: "parvovirus-b19",
    resistance: [],
    effective: [],
    strengths: [
      "Erythroid-precursor tropism (P antigen)",
      "Aplastic crisis",
      "Hydrops fetalis",
    ],
  },
  {
    section: "viruses",
    slug: "dengue-virus",
    resistance: [],
    effective: [],
    strengths: [
      "Four serotypes",
      "Antibody-dependent enhancement",
      "Plasma leakage (hemorrhagic fever)",
    ],
  },
  {
    section: "viruses",
    slug: "zika-virus",
    resistance: [],
    effective: [],
    strengths: [
      "Neurotropic (microcephaly)",
      "Sexual & vertical transmission",
      "Guillain-Barré association",
    ],
  },
  {
    section: "viruses",
    slug: "poliovirus",
    resistance: [],
    effective: [],
    strengths: [
      "Anterior horn motor-neuron tropism",
      "Very stable; fecal-oral",
      "Flaccid paralysis",
    ],
  },
  {
    section: "viruses",
    slug: "mumps-virus",
    resistance: [],
    effective: [],
    strengths: [
      "Parotid, testis & CNS tropism",
      "Airborne",
      "Orchitis / aseptic meningitis",
    ],
  },
  {
    section: "viruses",
    slug: "human-herpesvirus-6",
    resistance: [],
    effective: ["ganciclovir", "foscarnet"],
    strengths: [
      "Roseola infantum",
      "Latency in lymphocytes",
      "Reactivation in transplant",
    ],
  },
  {
    section: "viruses",
    slug: "kaposi-sarcoma-herpesvirus",
    resistance: [],
    effective: ["ganciclovir"],
    strengths: [
      "Latency in B cells",
      "vGPCR / vIL-6 drive angiogenesis",
      "Kaposi sarcoma & primary effusion lymphoma",
    ],
  },
  {
    section: "viruses",
    slug: "mers-cov",
    resistance: [],
    effective: ["remdesivir"],
    strengths: [
      "DPP4 entry receptor",
      "Dromedary-camel reservoir",
      "Severe pneumonia, high mortality",
    ],
  },
  {
    section: "viruses",
    slug: "crimean-congo-hemorrhagic-fever-virus",
    resistance: [],
    effective: ["ribavirin"],
    strengths: [
      "Tick-borne hemorrhagic fever",
      "Nosocomial spread via blood",
      "High mortality",
    ],
  },
  {
    section: "viruses",
    slug: "marburg-virus",
    resistance: [],
    effective: [],
    strengths: [
      "Filovirus hemorrhagic fever",
      "Bat / cave reservoir",
      "Suppresses innate immunity",
    ],
  },
  {
    section: "viruses",
    slug: "htlv-1",
    resistance: [],
    effective: [],
    strengths: [
      "Tax oncoprotein",
      "Adult T-cell leukemia/lymphoma",
      "HTLV-associated myelopathy",
    ],
  },
  {
    section: "viruses",
    slug: "hiv-2",
    resistance: ["efavirenz", "nevirapine"],
    effective: ["dolutegravir", "tenofovir", "emtricitabine", "darunavir"],
    strengths: [
      "Intrinsic NNRTI resistance",
      "Slower progression",
      "West-African origin",
    ],
  },

  // ---------- Fungi ----------
  {
    section: "fungi",
    slug: "candida-albicans",
    resistance: [],
    effective: [
      "fluconazole",
      "caspofungin",
      "micafungin",
      "anidulafungin",
      "amphotericin-b",
      "nystatin",
      "ibrexafungerp",
    ],
    strengths: [
      "Yeast–hyphal dimorphism",
      "Biofilm on devices",
      "Adhesins for tissue invasion",
    ],
  },
  {
    section: "fungi",
    slug: "candida-glabrata",
    resistance: ["fluconazole"],
    effective: [
      "caspofungin",
      "micafungin",
      "anidulafungin",
      "amphotericin-b",
    ],
    strengths: [
      "Rapid azole resistance (efflux pumps)",
      "Hardy haploid yeast",
    ],
  },
  {
    section: "fungi",
    slug: "candida-krusei",
    resistance: ["fluconazole"],
    effective: [
      "caspofungin",
      "micafungin",
      "anidulafungin",
      "voriconazole",
      "amphotericin-b",
    ],
    strengths: ["Intrinsic fluconazole resistance"],
  },
  {
    section: "fungi",
    slug: "candida-auris",
    resistance: ["fluconazole", "amphotericin-b"],
    effective: ["caspofungin", "micafungin", "anidulafungin", "ibrexafungerp"],
    strengths: [
      "Multidrug resistance",
      "Skin & surface colonization",
      "Outbreak persistence",
    ],
  },
  {
    section: "fungi",
    slug: "cryptococcus-neoformans",
    resistance: ["caspofungin"],
    effective: ["amphotericin-b", "flucytosine", "fluconazole"],
    strengths: [
      "Polysaccharide capsule (antiphagocytic)",
      "Melanin (laccase)",
      "Urease",
      "Grows at 37°C",
    ],
  },
  {
    section: "fungi",
    slug: "aspergillus-fumigatus",
    resistance: ["fluconazole"],
    effective: [
      "voriconazole",
      "isavuconazole",
      "posaconazole",
      "amphotericin-b",
    ],
    strengths: [
      "Angioinvasive hyphae",
      "Small airborne conidia",
      "Gliotoxin immunosuppression",
      "Thermotolerant",
    ],
  },
  {
    section: "fungi",
    slug: "rhizopus-arrhizus",
    resistance: ["fluconazole", "voriconazole", "caspofungin"],
    effective: ["amphotericin-b", "posaconazole", "isavuconazole"],
    strengths: [
      "Angioinvasive (vascular thrombosis)",
      "Thrives in acidic, iron-rich blood (DKA)",
      "Rapid growth",
    ],
  },
  {
    section: "fungi",
    slug: "histoplasma-capsulatum",
    resistance: [],
    effective: ["itraconazole", "amphotericin-b"],
    strengths: [
      "Intracellular in macrophages",
      "Thermal dimorphism",
      "Inhibits phagolysosome",
    ],
  },
  {
    section: "fungi",
    slug: "blastomyces-dermatitidis",
    resistance: [],
    effective: ["itraconazole", "amphotericin-b"],
    strengths: [
      "BAD1 adhesin",
      "Thermal dimorphism",
      "Broad-based budding",
    ],
  },
  {
    section: "fungi",
    slug: "coccidioides-immitis",
    resistance: [],
    effective: ["fluconazole", "itraconazole", "amphotericin-b"],
    strengths: [
      "Spherule/endospore tissue cycle",
      "Highly infectious arthroconidia",
    ],
  },
  {
    section: "fungi",
    slug: "sporothrix-schenckii",
    resistance: [],
    effective: ["itraconazole", "amphotericin-b"],
    strengths: [
      "Thermal dimorphism",
      "Traumatic implantation (thorns)",
      "Lymphocutaneous spread",
    ],
  },
  {
    section: "fungi",
    slug: "pneumocystis-jirovecii",
    resistance: ["caspofungin", "fluconazole", "amphotericin-b"],
    effective: ["tmp-smx", "pentamidine", "atovaquone", "dapsone"],
    strengths: [
      "Lacks ergosterol (azole/ampho resistant)",
      "Adheres to alveolar epithelium",
      "Cyst & trophic forms",
    ],
  },
  {
    section: "fungi",
    slug: "trichophyton-rubrum",
    resistance: [],
    effective: ["terbinafine", "itraconazole", "ciclopirox"],
    strengths: ["Keratinase (digests keratin)", "Chronic & relapsing"],
  },
  {
    section: "fungi",
    slug: "microsporum-canis",
    resistance: [],
    effective: ["griseofulvin", "terbinafine"],
    strengths: ["Keratinase", "Zoophilic spread (cats/dogs)"],
  },
  {
    section: "fungi",
    slug: "malassezia-furfur",
    resistance: [],
    effective: ["ketoconazole", "ciclopirox"],
    strengths: ["Lipid-dependent growth", "Skin commensal overgrowth"],
  },
  {
    section: "fungi",
    slug: "aspergillus-flavus",
    resistance: ["fluconazole"],
    effective: ["voriconazole", "isavuconazole", "amphotericin-b", "posaconazole"],
    strengths: [
      "Aflatoxin (hepatocarcinogen)",
      "Angioinvasive hyphae",
      "Sinusitis & keratitis",
    ],
  },
  {
    section: "fungi",
    slug: "aspergillus-niger",
    resistance: ["fluconazole"],
    effective: ["voriconazole", "amphotericin-b", "isavuconazole"],
    strengths: ["Otomycosis", "Calcium-oxalate crystals", "Black conidia"],
  },
  {
    section: "fungi",
    slug: "aspergillus-terreus",
    resistance: ["fluconazole", "amphotericin-b"],
    effective: ["voriconazole", "isavuconazole", "posaconazole"],
    strengths: [
      "Intrinsic amphotericin B resistance",
      "Angioinvasive",
      "Accessory aleurioconidia",
    ],
  },
  {
    section: "fungi",
    slug: "fusarium-solani",
    resistance: ["fluconazole", "caspofungin", "micafungin", "anidulafungin"],
    effective: ["voriconazole", "amphotericin-b", "posaconazole"],
    strengths: [
      "Echinocandin resistance",
      "Contact-lens keratitis",
      "Adventitious sporulation (fungemia)",
    ],
  },
  {
    section: "fungi",
    slug: "scedosporium-apiospermum",
    resistance: ["amphotericin-b", "fluconazole"],
    effective: ["voriconazole", "posaconazole"],
    strengths: [
      "Intrinsic amphotericin resistance",
      "Brain abscess after near-drowning",
      "Mycetoma",
    ],
  },
  {
    section: "fungi",
    slug: "lomentospora-prolificans",
    resistance: ["amphotericin-b", "fluconazole", "itraconazole", "caspofungin"],
    effective: ["voriconazole"],
    strengths: [
      "Pan-antifungal resistance",
      "Disseminated in neutropenia",
      "Very high mortality",
    ],
  },
  {
    section: "fungi",
    slug: "mucor-circinelloides",
    resistance: ["fluconazole", "voriconazole", "caspofungin"],
    effective: ["amphotericin-b", "posaconazole", "isavuconazole"],
    strengths: [
      "Angioinvasive aseptate hyphae",
      "Iron-dependent (DKA)",
      "Mucormycosis",
    ],
  },
  {
    section: "fungi",
    slug: "lichtheimia-corymbifera",
    resistance: ["fluconazole", "voriconazole", "caspofungin"],
    effective: ["amphotericin-b", "posaconazole", "isavuconazole"],
    strengths: [
      "Mucoralean angioinvasion",
      "Cutaneous after burns/trauma",
      "Rapid growth",
    ],
  },
  {
    section: "fungi",
    slug: "cunninghamella-bertholletiae",
    resistance: ["fluconazole", "voriconazole", "caspofungin"],
    effective: ["amphotericin-b", "posaconazole", "isavuconazole"],
    strengths: [
      "Aggressive pulmonary mucormycosis",
      "Angioinvasive",
      "Very high mortality",
    ],
  },
  {
    section: "fungi",
    slug: "candida-tropicalis",
    resistance: [],
    effective: [
      "fluconazole",
      "caspofungin",
      "micafungin",
      "anidulafungin",
      "amphotericin-b",
    ],
    strengths: [
      "Disseminates in neutropenia",
      "Biofilm",
      "Pseudohyphae",
    ],
  },
  {
    section: "fungi",
    slug: "candida-parapsilosis",
    resistance: [],
    effective: ["fluconazole", "amphotericin-b", "voriconazole", "caspofungin"],
    strengths: [
      "Biofilm on catheters & TPN lines",
      "Neonatal ICU fungemia",
      "Thrives on plastics",
    ],
  },
  {
    section: "fungi",
    slug: "candida-lusitaniae",
    resistance: ["amphotericin-b"],
    effective: ["fluconazole", "caspofungin", "micafungin"],
    strengths: ["Intrinsic amphotericin B resistance", "Pseudohyphae"],
  },
  {
    section: "fungi",
    slug: "candida-haemulonii",
    resistance: ["fluconazole", "amphotericin-b"],
    effective: ["caspofungin", "micafungin", "anidulafungin"],
    strengths: ["Multidrug resistance", "Frequently misidentified", "Nosocomial"],
  },
  {
    section: "fungi",
    slug: "candida-dubliniensis",
    resistance: [],
    effective: ["fluconazole", "caspofungin", "amphotericin-b"],
    strengths: [
      "Germ-tube positive (mimics C. albicans)",
      "Oral thrush in HIV",
    ],
  },
  {
    section: "fungi",
    slug: "cryptococcus-gattii",
    resistance: ["caspofungin"],
    effective: ["amphotericin-b", "flucytosine", "fluconazole"],
    strengths: [
      "Polysaccharide capsule",
      "Mass-like disease in immunocompetent hosts",
      "Eucalyptus association",
    ],
  },
  {
    section: "fungi",
    slug: "trichosporon-asahii",
    resistance: ["caspofungin", "micafungin", "anidulafungin", "amphotericin-b"],
    effective: ["voriconazole", "fluconazole"],
    strengths: [
      "Echinocandin resistance (no glucan target)",
      "Disseminated in neutropenia",
      "Cross-reacts with cryptococcal antigen",
    ],
  },
  {
    section: "fungi",
    slug: "saccharomyces-cerevisiae",
    resistance: [],
    effective: ["fluconazole", "amphotericin-b", "caspofungin"],
    strengths: [
      "Probiotic origin (boulardii)",
      "Translocation fungemia from lines",
      "Generally low virulence",
    ],
  },
  {
    section: "fungi",
    slug: "malassezia-globosa",
    resistance: [],
    effective: ["ketoconazole", "ciclopirox", "itraconazole"],
    strengths: ["Lipid-dependent", "Dandruff / seborrheic dermatitis"],
  },
  {
    section: "fungi",
    slug: "paracoccidioides-brasiliensis",
    resistance: [],
    effective: ["itraconazole", "tmp-smx", "amphotericin-b"],
    strengths: [
      "'Captain's wheel' multiple budding",
      "Mucocutaneous & pulmonary disease",
      "Thermal dimorphism",
    ],
  },
  {
    section: "fungi",
    slug: "talaromyces-marneffei",
    resistance: [],
    effective: ["amphotericin-b", "itraconazole", "voriconazole"],
    strengths: [
      "Fission yeast in tissue",
      "Disseminated in HIV (SE Asia)",
      "Molluscum-like papules",
    ],
  },
  {
    section: "fungi",
    slug: "fonsecaea-pedrosoi",
    resistance: [],
    effective: ["itraconazole", "terbinafine", "flucytosine"],
    strengths: [
      "Sclerotic ('copper-penny') bodies",
      "Chromoblastomycosis",
      "Dematiaceous melanin",
    ],
  },
  {
    section: "fungi",
    slug: "exophiala-dermatitidis",
    resistance: [],
    effective: ["voriconazole", "itraconazole", "amphotericin-b"],
    strengths: [
      "Black yeast (melanin)",
      "Phaeohyphomycosis",
      "CF airway colonization",
    ],
  },
  {
    section: "fungi",
    slug: "cladophialophora-bantiana",
    resistance: [],
    effective: ["voriconazole", "amphotericin-b", "flucytosine"],
    strengths: [
      "Neurotropic dematiaceous mold",
      "Brain abscess in immunocompetent hosts",
      "Melanized cell wall",
    ],
  },
  {
    section: "fungi",
    slug: "trichophyton-tonsurans",
    resistance: [],
    effective: ["griseofulvin", "terbinafine", "itraconazole"],
    strengths: [
      "Endothrix 'black-dot' tinea capitis",
      "Anthropophilic",
      "Keratinase",
    ],
  },
  {
    section: "fungi",
    slug: "trichophyton-mentagrophytes",
    resistance: [],
    effective: ["terbinafine", "itraconazole", "griseofulvin"],
    strengths: ["Zoophilic inflammatory tinea", "Spiral hyphae", "Keratinase"],
  },
  {
    section: "fungi",
    slug: "epidermophyton-floccosum",
    resistance: [],
    effective: ["terbinafine", "itraconazole", "griseofulvin"],
    strengths: [
      "Infects skin & nails (never hair)",
      "Club-shaped macroconidia",
      "Keratinase",
    ],
  },
  {
    section: "fungi",
    slug: "coccidioides-posadasii",
    resistance: [],
    effective: ["fluconazole", "itraconazole", "amphotericin-b"],
    strengths: [
      "Spherule / endospore tissue cycle",
      "Highly infectious arthroconidia",
      "Valley fever",
    ],
  },
  {
    section: "fungi",
    slug: "blastomyces-gilchristii",
    resistance: [],
    effective: ["itraconazole", "amphotericin-b"],
    strengths: [
      "Broad-based budding",
      "Pulmonary & cutaneous disease",
      "Thermal dimorphism",
    ],
  },

  // ---------- Parasites ----------
  {
    section: "parasites",
    slug: "plasmodium-falciparum",
    resistance: ["chloroquine"],
    effective: [
      "artemether-lumefantrine",
      "atovaquone-proguanil",
      "quinine",
      "mefloquine",
    ],
    strengths: [
      "PfEMP1 antigenic variation",
      "Cytoadherence & sequestration",
      "Infects red cells of all ages (high parasitemia)",
    ],
  },
  {
    section: "parasites",
    slug: "plasmodium-vivax",
    resistance: [],
    effective: [
      "chloroquine",
      "hydroxychloroquine",
      "primaquine",
      "tafenoquine",
    ],
    strengths: ["Dormant hepatic hypnozoites (relapse)", "Duffy-antigen entry"],
  },
  {
    section: "parasites",
    slug: "plasmodium-ovale",
    resistance: [],
    effective: ["chloroquine", "primaquine", "tafenoquine"],
    strengths: ["Dormant hypnozoites (relapse)"],
  },
  {
    section: "parasites",
    slug: "plasmodium-malariae",
    resistance: [],
    effective: ["chloroquine", "hydroxychloroquine"],
    strengths: ["Decades-long low-grade parasitemia"],
  },
  {
    section: "parasites",
    slug: "toxoplasma-gondii",
    resistance: [],
    effective: ["pyrimethamine-sulfadiazine", "spiramycin", "atovaquone"],
    strengths: [
      "Intracellular (any nucleated cell)",
      "Latent tissue cysts",
      "Crosses the placenta",
    ],
  },
  {
    section: "parasites",
    slug: "cryptosporidium-parvum",
    resistance: [],
    effective: ["nitazoxanide"],
    strengths: [
      "Chlorine-resistant oocysts",
      "Autoinfection",
      "Low infectious dose",
    ],
  },
  {
    section: "parasites",
    slug: "babesia-microti",
    resistance: [],
    effective: ["atovaquone", "azithromycin", "quinine"],
    strengths: ["Intraerythrocytic replication", "Severe in asplenia"],
  },
  {
    section: "parasites",
    slug: "giardia-lamblia",
    resistance: [],
    effective: ["metronidazole", "tinidazole", "nitazoxanide"],
    strengths: [
      "Chlorine-resistant cysts",
      "Ventral attachment disc",
      "Surface-antigen variation",
    ],
  },
  {
    section: "parasites",
    slug: "trichomonas-vaginalis",
    resistance: [],
    effective: ["metronidazole", "tinidazole"],
    strengths: ["Flagellar motility", "Adheres to urogenital epithelium"],
  },
  {
    section: "parasites",
    slug: "entamoeba-histolytica",
    resistance: [],
    effective: ["metronidazole", "tinidazole", "paromomycin"],
    strengths: [
      "Contact-dependent cytolysis",
      "Hardy cysts",
      "Lectin-mediated adherence",
    ],
  },
  {
    section: "parasites",
    slug: "trypanosoma-cruzi",
    resistance: [],
    effective: ["benznidazole", "nifurtimox"],
    strengths: [
      "Intracellular amastigotes",
      "Lifelong persistence",
      "Immune-mediated tissue damage",
    ],
  },
  {
    section: "parasites",
    slug: "trypanosoma-brucei",
    resistance: [],
    effective: ["pentamidine", "nifurtimox", "eflornithine"],
    strengths: [
      "VSG antigenic variation (evades antibody)",
      "Crosses the blood-brain barrier",
    ],
  },
  {
    section: "parasites",
    slug: "leishmania-donovani",
    resistance: [],
    effective: [
      "amphotericin-b",
      "miltefosine",
      "paromomycin",
      "sodium-stibogluconate",
    ],
    strengths: [
      "Survives inside macrophages",
      "Inhibits phagolysosome killing",
      "Sandfly reservoir",
    ],
  },
  {
    section: "parasites",
    slug: "naegleria-fowleri",
    resistance: [],
    effective: ["miltefosine", "amphotericin-b"],
    strengths: ["Ascends the cribriform plate", "Rapid tissue destruction"],
  },
  {
    section: "parasites",
    slug: "ascaris-lumbricoides",
    resistance: [],
    effective: ["albendazole", "mebendazole", "ivermectin", "pyrantel-pamoate"],
    strengths: [
      "Durable eggs in soil",
      "Larval lung migration",
      "Massive worm burden / obstruction",
    ],
  },
  {
    section: "parasites",
    slug: "enterobius-vermicularis",
    resistance: [],
    effective: ["mebendazole", "albendazole", "pyrantel-pamoate"],
    strengths: ["Nocturnal perianal egg-laying", "Autoinfection"],
  },
  {
    section: "parasites",
    slug: "strongyloides-stercoralis",
    resistance: [],
    effective: ["ivermectin", "albendazole"],
    strengths: [
      "Autoinfection → hyperinfection",
      "Free-living & parasitic cycles",
    ],
  },
  {
    section: "parasites",
    slug: "trichinella-spiralis",
    resistance: [],
    effective: ["albendazole", "mebendazole"],
    strengths: ["Encysts in striated muscle", "Nurse-cell formation"],
  },
  {
    section: "parasites",
    slug: "necator-americanus",
    resistance: [],
    effective: ["albendazole", "mebendazole", "pyrantel-pamoate"],
    strengths: [
      "Skin-penetrating larvae",
      "Cutting plates; chronic blood loss",
    ],
  },
  {
    section: "parasites",
    slug: "wuchereria-bancrofti",
    resistance: [],
    effective: ["ivermectin", "albendazole", "diethylcarbamazine"],
    strengths: [
      "Lymphatic obstruction",
      "Wolbachia endosymbiont",
      "Nocturnal microfilaremia",
    ],
  },
  {
    section: "parasites",
    slug: "brugia-malayi",
    resistance: [],
    effective: ["diethylcarbamazine", "albendazole", "ivermectin"],
    strengths: ["Lymphatic obstruction", "Wolbachia endosymbiont"],
  },
  {
    section: "parasites",
    slug: "onchocerca-volvulus",
    resistance: [],
    effective: ["ivermectin"],
    strengths: ["Wolbachia endosymbiont", "Microfilarial eye invasion"],
  },
  {
    section: "parasites",
    slug: "schistosoma-mansoni",
    resistance: [],
    effective: ["praziquantel"],
    strengths: [
      "Coats itself in host antigens",
      "Long-lived in mesenteric veins",
      "Egg-induced granulomas",
    ],
  },
  {
    section: "parasites",
    slug: "schistosoma-haematobium",
    resistance: [],
    effective: ["praziquantel"],
    strengths: [
      "Host-antigen masking",
      "Bladder egg deposition (cancer risk)",
    ],
  },
  {
    section: "parasites",
    slug: "fasciola-hepatica",
    resistance: [],
    effective: ["triclabendazole"],
    strengths: ["Migrates through liver parenchyma", "Long-lived bile-duct fluke"],
  },
  {
    section: "parasites",
    slug: "paragonimus-westermani",
    resistance: [],
    effective: ["praziquantel", "triclabendazole"],
    strengths: ["Encysts in lung", "Tissue migration"],
  },
  {
    section: "parasites",
    slug: "taenia-solium",
    resistance: [],
    effective: ["praziquantel", "albendazole"],
    strengths: [
      "Cysticerci (including CNS)",
      "Scolex with hooks",
      "Egg autoinfection",
    ],
  },
  {
    section: "parasites",
    slug: "echinococcus-granulosus",
    resistance: [],
    effective: ["albendazole"],
    strengths: [
      "Expanding hydatid cyst",
      "Daughter cysts",
      "Anaphylaxis on rupture",
    ],
  },
  {
    section: "parasites",
    slug: "sarcoptes-scabiei",
    resistance: [],
    effective: ["permethrin", "ivermectin"],
    strengths: [
      "Burrows in stratum corneum",
      "Crusted scabies (huge burden) in immunocompromised",
    ],
  },
  {
    section: "parasites",
    slug: "pediculus-humanus-capitis",
    resistance: [],
    effective: ["permethrin", "ivermectin", "malathion"],
    strengths: ["Cements nits to hair shafts", "Direct-contact spread"],
  },
  {
    section: "parasites",
    slug: "plasmodium-knowlesi",
    resistance: [],
    effective: ["artemether-lumefantrine", "chloroquine", "atovaquone-proguanil"],
    strengths: [
      "24-hour replication cycle (rapid)",
      "Zoonotic (macaque reservoir)",
      "High parasitemia",
    ],
  },
  {
    section: "parasites",
    slug: "cyclospora-cayetanensis",
    resistance: [],
    effective: ["tmp-smx"],
    strengths: [
      "Produce-borne oocysts",
      "Prolonged relapsing diarrhea",
      "Acid-fast oocysts",
    ],
  },
  {
    section: "parasites",
    slug: "cystoisospora-belli",
    resistance: [],
    effective: ["tmp-smx"],
    strengths: [
      "Chronic diarrhea in AIDS",
      "Acid-fast oocysts",
      "Autoinfection",
    ],
  },
  {
    section: "parasites",
    slug: "balantidium-coli",
    resistance: [],
    effective: ["tetracycline", "metronidazole", "nitazoxanide"],
    strengths: [
      "Only ciliate to infect humans",
      "Invades colonic mucosa",
      "Pig reservoir",
    ],
  },
  {
    section: "parasites",
    slug: "leishmania-braziliensis",
    resistance: [],
    effective: ["amphotericin-b", "sodium-stibogluconate", "miltefosine"],
    strengths: [
      "Mucocutaneous destruction (espundia)",
      "Survives in macrophages",
      "Late metastatic spread",
    ],
  },
  {
    section: "parasites",
    slug: "leishmania-infantum",
    resistance: [],
    effective: ["amphotericin-b", "miltefosine", "sodium-stibogluconate"],
    strengths: [
      "Visceral disease (infantile / HIV)",
      "Macrophage amastigotes",
      "Dog reservoir",
    ],
  },
  {
    section: "parasites",
    slug: "dientamoeba-fragilis",
    resistance: [],
    effective: ["metronidazole", "tinidazole", "paromomycin"],
    strengths: [
      "No cyst stage",
      "Chronic GI symptoms",
      "Binucleate trophozoite",
    ],
  },
  {
    section: "parasites",
    slug: "enterocytozoon-bieneusi",
    resistance: [],
    effective: ["fumagillin", "nitazoxanide"],
    strengths: [
      "Microsporidian (obligate intracellular)",
      "Chronic diarrhea in AIDS",
      "Polar-tubule invasion",
    ],
  },
  {
    section: "parasites",
    slug: "acanthamoeba-castellanii",
    resistance: [],
    effective: ["miltefosine"],
    strengths: [
      "Contact-lens keratitis",
      "Granulomatous amoebic encephalitis",
      "Disinfectant-resistant cysts",
    ],
  },
  {
    section: "parasites",
    slug: "balamuthia-mandrillaris",
    resistance: [],
    effective: ["miltefosine"],
    strengths: [
      "Granulomatous amoebic encephalitis",
      "Infects immunocompetent hosts",
      "Slowly progressive",
    ],
  },
  {
    section: "parasites",
    slug: "ancylostoma-duodenale",
    resistance: [],
    effective: ["albendazole", "mebendazole", "pyrantel-pamoate"],
    strengths: [
      "Cutting teeth (mucosal blood loss)",
      "Iron-deficiency anemia",
      "Skin-penetrating larvae",
    ],
  },
  {
    section: "parasites",
    slug: "toxocara-canis",
    resistance: [],
    effective: ["albendazole", "mebendazole"],
    strengths: [
      "Visceral & ocular larva migrans",
      "Dead-end human host",
      "Marked eosinophilia",
    ],
  },
  {
    section: "parasites",
    slug: "loa-loa",
    resistance: [],
    effective: ["diethylcarbamazine", "albendazole"],
    strengths: [
      "Calabar swellings",
      "Conjunctival migration ('eye worm')",
      "Deer-fly borne",
    ],
  },
  {
    section: "parasites",
    slug: "trichuris-trichiura",
    resistance: [],
    effective: ["mebendazole", "albendazole", "ivermectin"],
    strengths: [
      "Whip shape threads into mucosa",
      "Rectal prolapse (heavy burden)",
      "Chronic diarrhea",
    ],
  },
  {
    section: "parasites",
    slug: "angiostrongylus-cantonensis",
    resistance: [],
    effective: ["albendazole"],
    strengths: [
      "Eosinophilic meningitis",
      "Snail / slug acquired",
      "CNS larval migration",
    ],
  },
  {
    section: "parasites",
    slug: "clonorchis-sinensis",
    resistance: [],
    effective: ["praziquantel", "albendazole"],
    strengths: [
      "Biliary residence",
      "Cholangiocarcinoma risk",
      "Raw freshwater fish",
    ],
  },
  {
    section: "parasites",
    slug: "opisthorchis-viverrini",
    resistance: [],
    effective: ["praziquantel"],
    strengths: [
      "Biliary fluke",
      "Cholangiocarcinoma (SE Asia)",
      "Raw fish acquired",
    ],
  },
  {
    section: "parasites",
    slug: "schistosoma-japonicum",
    resistance: [],
    effective: ["praziquantel"],
    strengths: [
      "Highest egg output",
      "Hepatosplenic disease / Katayama fever",
      "Host-antigen masking",
    ],
  },
  {
    section: "parasites",
    slug: "diphyllobothrium-latum",
    resistance: [],
    effective: ["praziquantel"],
    strengths: [
      "Competes for vitamin B12 (megaloblastic anemia)",
      "Largest human tapeworm",
      "Raw freshwater fish",
    ],
  },
  {
    section: "parasites",
    slug: "taenia-saginata",
    resistance: [],
    effective: ["praziquantel"],
    strengths: [
      "Beef-acquired",
      "Motile proglottids",
      "No cysticercosis (unlike T. solium)",
    ],
  },
  {
    section: "parasites",
    slug: "hymenolepis-nana",
    resistance: [],
    effective: ["praziquantel"],
    strengths: [
      "Autoinfection (no intermediate host)",
      "Most common tapeworm in children",
    ],
  },
  {
    section: "parasites",
    slug: "echinococcus-multilocularis",
    resistance: [],
    effective: ["albendazole"],
    strengths: [
      "Invasive alveolar (tumor-like) cysts",
      "Fox / rodent cycle",
      "Often inoperable",
    ],
  },
  {
    section: "parasites",
    slug: "pediculus-humanus-corporis",
    resistance: [],
    effective: ["permethrin", "malathion"],
    strengths: [
      "Vector: epidemic typhus, trench fever, relapsing fever",
      "Lives in clothing seams",
    ],
  },
  {
    section: "parasites",
    slug: "phthirus-pubis",
    resistance: [],
    effective: ["permethrin", "malathion"],
    strengths: [
      "Crab louse of coarse hair",
      "Maculae ceruleae",
      "Sexually transmitted",
    ],
  },
  {
    section: "parasites",
    slug: "demodex-folliculorum",
    resistance: [],
    effective: ["permethrin", "ivermectin"],
    strengths: [
      "Facial follicle mite",
      "Rosacea & blepharitis",
      "Normally commensal",
    ],
  },
];

export function getPharmacologyFor(
  section: SectionKey,
  slug: string,
): MicrobePharmacology | undefined {
  return MICROBE_PHARMACOLOGY.find(
    (m) => m.section === section && m.slug === slug,
  );
}

export function getMicrobesEffectiveFor(
  drugSlug: string,
): MicrobePharmacology[] {
  return MICROBE_PHARMACOLOGY.filter((m) => m.effective.includes(drugSlug));
}

export function getMicrobesResistantTo(
  drugSlug: string,
): MicrobePharmacology[] {
  return MICROBE_PHARMACOLOGY.filter((m) => m.resistance.includes(drugSlug));
}
